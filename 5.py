...
class GetZhuanLan(object):
    ...
    def __init__(self, zhuanlan_name, thread_number,proxy = False,page =0, time_limit = 0 ):
    ...
    def start(self):
        self.login(self.s_people,self.header,"password","user",self.need_IP) # you need to input user and password here
        self.login(self.s_zhuanlan,self.header,"password","user",self.need_IP)  # you need to input user and password here,we need you account to reduce the change of block
        ...
        self.get_follow_list(self.zhuanlan_name)
    ...
    def get_follow_list(self, zhuanlan_name):
    ...
    def worker(self):
        while True:
            ...
            if item is None:
                time.sleep(3)
                continue
            self.deal_follows(item)
            self.q.task_done()


    def deal_follows(self,follows):
        try:        
            for i in follows:
                ...
                if focus_zhuanlan == []:
                    print(i[1], "作弊")
                person_basic = { "person_hash":i[0],"person_name":i[1], "person_hash":i[2],"person_url":i[3], "person_bio":i[4], "person_description":i[5],"person_focuszhuanlan":focus_zhuanlan, "person_page":person_page, "time":time.time(), "localtime":time.strftime('%X %x %Z')}
...
##        stop_url = 'http://192.168.1.1/userRpm/StatusRpm.htm?Disconnect=%B6%CF%20%CF%DF&wan=1'
        ...
        ip_before = requests.get('http://jsonip.com')
        ...
        while True:
            if time.time() - self.start_time < 1200:
                ...
                break
            need_proxy = input('need proxy,input y,otherwise n')
            if need_proxy == 'y':
                self.proxies = {'http': 'http://127.0.0.1:8087','https': 'http://127.0.0.1:8087'}
                self.s_people.proxies.update(self.proxies)
                self.s_zhuanlan.proxies.update(self.proxies)
            else:
            ...
            try:
                ip_after = requests.get('http://jsonip.com')
...
GetZhuanLan('wontfallinyourlap',5).start() # input the zhuanlan you want to study and numbers of threads
